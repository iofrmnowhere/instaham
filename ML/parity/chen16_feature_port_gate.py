...
from ML.parity.compare import CHEN16_EXACT as EXACT  # noqa: E402
from ML.parity.compare import CHEN16_REL_LOOSE as REL_1E6  # noqa: E402
from ML.parity.compare import CHEN16_REL_TIGHT as REL_1E3  # noqa: E402

REPO_ROOT = pathlib.Path(__file__).resolve().parents[2]
MASK_ROOT = REPO_ROOT / "Instaham" / "PIGRGB-Weight" / "MASK_3394"

# docs/metrics-phase/5-parity-tests.md task 1: EXACT/REL_1E3/REL_1E6 finalized and moved
# to ML/parity/compare.py (CHEN16_EXACT/CHEN16_REL_TIGHT/CHEN16_REL_LOOSE) so this gate and
# the parity test read the same buckets. Aliased back to the original names here so the
...
def emit_python_golden(limit: int, out: pathlib.Path) -> int:
    """docs/metrics-phase/5-parity-tests.md task 7's committed-golden fallback: writes
    {mask_relpath: {feature: value}} computed by the Python oracle ONLY, for the same
    sample_masks() selection this gate already uses. test/parity/parity_test.dart reads
    this file and re-runs ONLY the native CLI at test time (no python subprocess from
    Dart -- Process.run('python', ...) was found to resolve a different, broken
    interpreter than the one this script itself runs under on this machine, see the
    phase document's risk section), so the golden must be regenerated by re-running this
    function whenever CHEN16_NOHEIGHT, extract_chen16_features, or the mask corpus
    changes:

        python -m ML.parity.chen16_feature_port_gate --emit-python-golden \\
            test/fixtures/parity/chen16_python_golden.json --limit 500
    """
    if not MASK_ROOT.exists():
        print(f"mask corpus not found at {MASK_ROOT}", file=sys.stderr)
        return 2
    masks = sample_masks(limit)
    golden: dict[str, dict[str, float] | None] = {}
    for mask_path in masks:
        rel = str(mask_path.relative_to(REPO_ROOT)).replace("\\", "/")
        py = run_python(mask_path)
        golden[rel] = py
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(golden, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(f"wrote {out} ({len(golden)} masks)")
    return 0
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        type=pathlib.Path,
        default=None,
        help="write the Python-only golden JSON the Dart parity test's fallback reads, "
        "instead of running the C++ vs Python comparison in-process",
    )
    args = ap.parse_args()

    if args.emit_python_golden is not None:
        return emit_python_golden(args.limit, args.emit_python_golden)
