...
| 4 | Validation: jitter, parity, and the on-device rebuild | **deferred 2026-09-18, no longer gated** — off-device jitter done in phase 3; the on-device F53 re-mark and full per-image table remain outstanding. [`fix-4.md`](fix-4.md) phase 2 has since rejected the normalize-first composition path (measured, not adopted — [fix-phase-4-2-results.md](fix-phase-4-2-results.md)), so this phase's code path is once again the only one in play and the earlier reason to defer (risk of validating a path that might not survive fix-4's gate) no longer applies. Still deferred because it was left that way, not restarted here; resume on its own terms whenever this round is picked back up. | [fix-phase-3/4-validation.md](fix-phase-3/4-validation.md) |
